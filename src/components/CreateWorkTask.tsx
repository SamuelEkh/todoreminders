...
const CreateWorkTask: React.FC<Props> = ({ socket, setPopupTrigger }) => {
  ...
  const removeClickHandler = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    // const targetTitle = target.previousElementSibling?.lastElementChild?.textContent;
  ...
  const changeHandler = (e: React.ChangeEvent<HTMLInputElement>) => {
    switch (e.target.className) {
      ...
      case 'subtask-form__input':
        ...
        break;
      case 'create-task__deadline-input':
  ...
  return (
    <>
      <form className="create-task" onSubmit={handleSubmit}>
        ...
        <div className="create-task__deadline-header">Deadline: </div>
        <input className="create-task__deadline-input" type="date" onChange={changeHandler} value={deadline} />
        {allSubTaskElements()}
